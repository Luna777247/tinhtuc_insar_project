...
var landslideSAR = diff91.abs().gt(3.0)
  .and(slope.gt(20)).and(slope.lt(55))
  .and(flood55.not());  // Loại trùng với ngập

// ============================================================
// 6b. XÁC NHẬN BẰNG NDVI (Sentinel-2)
// ============================================================

print("\n4b. Xác nhận bằng NDVI (Sentinel-2)...");

// Sentinel-2 NDVI trước và sau sự kiện
...
var s2Post = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED")
  .filterBounds(AOI)
  .filterDate("2025-10-02", "2025-10-07")  // Sau sự kiện 1-2 ngày
  .filter(ee.Filter.lt("CLOUDY_PIXEL_PERCENTAGE", 30))
  .select(["B4", "B8"])
  .median();

// Tính NDVI: (NIR - RED) / (NIR + RED)
var ndviPre = s2Pre.normalizedDifference(["B8", "B4"]).rename("NDVI_pre");
var ndviPost = s2Post.normalizedDifference(["B8", "B4"]).rename("NDVI_post");
var ndviDiff = ndviPost.subtract(ndviPre).rename("NDVI_diff");

// NDVI giảm > 0.2 = thực vật bị phá hủy (dấu hiệu sạt lở)
var ndviMask = ndviDiff.lt(-0.2);

// Landslide cuối cùng = SAR detection AND NDVI confirmation
var landslide91 = landslideSAR.and(ndviMask);

print("  ✓ Landslide SAR candidates:", landslideSAR.selfMask().reduceRegion({
  reducer: ee.Reducer.sum(), geometry: AOI, scale: 30, maxPixels: 1e9
}).get("VH_diff"));

print("  ✓ Landslide confirmed (NDVI < -0.2):", landslide91.selfMask().reduceRegion({
  reducer: ee.Reducer.sum(), geometry: AOI, scale: 30, maxPixels: 1e9
}).get("VH_diff"));
...
Map.addLayer(landslide91.selfMask(), {
  palette: ["#A32D2D"],
  opacity: 0.8
}, "⛰️ Sạt lở đã xác nhận (SAR + NDVI)");

Map.addLayer(landslideSAR.selfMask(), {
  palette: ["#FF8C00"],
  opacity: 0.5
}, "🔶 Sạt lở SAR chưa xác nhận (chỉ SAR)");

Map.addLayer(ndviDiff, {
  min: -0.5,
  max: 0.5,
  palette: ["#A32D2D", "#FFFACD", "#228B22"]
}, "🌿 Thay đổi NDVI (đỏ=giảm)");
